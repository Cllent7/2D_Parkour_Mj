这是基于Unity开发的2D横版跑酷游戏，复刻《天天酷跑》核心玩法，以单例模式、对象池、观察者模式构建低耦合架构，含角色控制、无限地图、双关卡挑战等完整模块。角色操作直观：点击屏幕右侧触发跳跃（支持二段跳，高度一致且动画同步切换），点击左侧触发下滑（蜷缩降低碰撞体积以穿越矮障碍），单例管理确保全局唯一控制与无延迟响应。无限地图通过对象池管理地图片段预制体，随玩家距离动态生成新片段（达阈值触发）并卸载左侧旧片段，实现无缝奔跑与性能优化。双关卡差异化设计：第一关需累计50分过关，含常规障碍与高低地形；第二关为无限循环模式，以死亡距离为记录，新增伸缩障碍、随机导弹等高阶挑战，障碍系统通过工厂模式+对象池统一管控生成与回收，导弹沿固定轨迹飞行且碰撞精准。数据与UI系统完善：分数统计用观察者模式，收集金币实时更新并触发数字弹窗+音效反馈；排行榜以ScriptableObject为载体，存储展示最高记录、近三次距离及累计金币，主菜单可查；UI涵盖开始菜单、游戏界面与结算弹窗，支持关卡切换、数据实时显示及重新开始等交互。整体结合ScriptableObject与单例模式实现稳定数据存储，架构清晰可扩展，完整还原经典跑酷体验。
